...
export async function installBundledAssets(
  ...
  const { bundledDir, targetDir, label, fileFilter } = options;

  log.info(`Copying default ${label} from: ${bundledDir}`);
  log.info(`Installing to: ${targetDir}`);

  await copyDirectory(bundledDir, targetDir, targetDir, label, fileFilter);
...
async function copyDirectory(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await copyDirectory(srcPath, destPath, baseDir, label, fileFilter);
