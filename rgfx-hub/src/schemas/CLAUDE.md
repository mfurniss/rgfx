...
- `video.ts` - Video streaming effect. Hub-only schema (no ESP32 counterpart in defaults.json). Props: `file` (path, defaults to empty string), `loop` (boolean). The hub intercepts this effect and delegates to VideoPlayer instead of sending JSON to drivers. Exports `fieldTypes` (`file` → `'videoFile'`) and `layoutConfig`.
