...
- `system-monitor.ts` — Monitors firmware versions, network status, UDP stats. Firmware versions come from `manifest.json` bundled alongside the `.bin` files in `assets/esp32/firmware/`. The manifest version must match the version baked into the firmware binaries (generated by `esp32/inject_version.py` at build time) — a mismatch causes the "new firmware available" banner to persist after flashing.
