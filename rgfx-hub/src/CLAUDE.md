...
- `launch-script-updater.ts` — Updates only the `ROM_PATH` line in the existing launch script, preserving all other user customizations (MAME_PATH, comments, etc.). Uses platform-specific regex patterns to match and replace the line. Used by the `updateMameRomsDirectory` IPC handler when the user saves a new ROM path in Settings.
