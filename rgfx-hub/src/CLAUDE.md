...
- `mame-detector.ts` — Detects MAME version by running `mame -version`. When a directory is provided, looks for the executable there. When empty, auto-detects from common install paths (Homebrew on macOS, `C:\mame` on Windows, PATH). Returns `{ version, detectedPath }` (version is string or null). 5-second timeout per candidate.
