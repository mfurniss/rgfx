...
- `video-player.ts` — Transcodes video files using ffmpeg (`child_process.spawn`) and streams raw RGB24 frames to ESP32 matrix drivers via binary UDP. Groups drivers by resolution, spawns one ffmpeg per unique `WxH`. Supports crop (default, preserves aspect ratio) and stretch fit modes via ffmpeg scale+crop filters. Handles frame pacing (`-re` flag + interval timer), fragment assembly, rotation-aware dimension calculation, and automatic strip driver exclusion. Clears pending frames on stop to prevent stale frame flash. Exports `VideoPlayer` class, `VideoFitMode` type, and pure utility functions (`resolveVideoPath`, `getDriverMatrixDimensions`, `buildVideoPacket`).
