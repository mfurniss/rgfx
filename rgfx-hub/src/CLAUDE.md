...
- `preload.ts` — Exposes secure IPC API to renderer via `window.rgfx` (47 methods: invoke, push, and send channels)
...
- `launch-script-updater.ts` — Updates a single variable line in the existing launch script, preserving all other user customizations. Generic `updateLaunchScriptVariable(variableName, value)` builds platform-specific regex and replacement templates. Convenience wrappers: `updateLaunchScriptRomPath()` (used by `updateMameRomsDirectory` IPC handler) and `updateLaunchScriptMamePath()` (used by `updateMameDirectory` IPC handler). Tests verify paths with spaces produce valid quoting and that other lines are unchanged.
...
- `system-monitor.ts` — Monitors firmware versions, network status, UDP stats, ffmpeg availability (`setFfmpegAvailable()`), driver fallback activity (`setDriverFallbackActive()` — session-sticky, stays true once fallback fires), and MAME version (`setMameVersion()`). Provides `getFullStatus()` method for assembling complete system status (includes `mameVersion` field). Includes `startUpdateChecker()` which checks GitHub releases for newer hub versions (5s delay, single check). Update availability is included in `SystemStatus.updateAvailable` as a release URL string. Firmware versions come from `manifest.json` bundled alongside the `.bin` files in `assets/esp32/firmware/`. The manifest version must match the version baked into the firmware binaries (generated by `esp32/inject_version.py` at build time) — a mismatch causes the "new firmware available" banner to persist after flashing. Version comparison uses the `semver` package for proper semver handling (prerelease, build metadata). Dev build detection (version contains `-dev`) is handled in both main process (`firmware-version-service.ts`) and renderer (`firmware-helpers.ts`).
- `mame-detector.ts` — Detects MAME version by running `mame -version`. When a directory is provided, looks for the executable there. When empty, auto-detects from common install paths (Homebrew on macOS, `C:\mame` on Windows, PATH). Returns `{ version, detectedPath }` (version is string or null). 5-second timeout per candidate. On startup, the main process auto-detects with an empty directory; the renderer then syncs the user's saved `mameDirectory` (from Zustand/localStorage) via `updateMameDirectory` IPC to re-detect from the configured path, ensuring non-standard install locations are found after restart.
- `video-player.ts` — Transcodes video files using ffmpeg (`child_process.spawn`) and streams raw RGB24 frames to ESP32 matrix drivers via binary UDP. Groups drivers by resolution, spawns one ffmpeg per unique `WxH`. Supports crop (default, preserves aspect ratio) and stretch fit modes via ffmpeg scale+crop filters. Uses ffmpeg `-re` flag for native frame pacing (no separate pace timer). Handles fragment assembly, rotation-aware dimension calculation, and automatic strip driver exclusion. Exports `VideoPlayer` class, `VideoFitMode` type, and pure utility functions (`resolveVideoPath`, `getDriverMatrixDimensions`, `buildVideoPacket`). Detection uses `ffmpeg -version` execution (not `which`) with fallback to absolute Homebrew paths, since Electron apps on macOS don't inherit the shell PATH.
...
- **Type `vi.fn()` generics explicitly** when assigning to typed variables — TypeScript 6 is stricter about `Mock<Procedure | Constructable>` not being assignable to specific function types. Use `vi.fn<(arg: Type) => ReturnType>()` or cast at the call site.
...
- `types/` — Shared types split into focused modules:
  - `types/driver.ts` — Driver, telemetry, and LED config types (LEDChipset supports WS2812B/WS2811/SK6812/WS2814, DriverTelemetry includes `ledHealthy?: boolean`, `discoveryMethod?: string`)
  - `types/system.ts` — System status and monitoring types (`SystemStatus` includes `mameVersion?: string`)
