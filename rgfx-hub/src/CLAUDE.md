...
- `mame-detector.ts` — Detects MAME version by running `mame -version`. When a directory is provided, looks for the executable there. When empty, auto-detects from common install paths (Homebrew on macOS, `C:\mame` on Windows, PATH). Returns version string (e.g. "0.286") or null if not found. 5-second timeout per candidate.
