...
| `ambilight.lua` | Screen edge color sampling — uses single-pass direct-to-zone accumulation with precomputed zone maps, flat arrays (zero per-frame allocations), inlined pixel access, and brightness LUT. Averages over N frames, emits `rgfx/ambilight/frame` events with 12-bit color zones |
