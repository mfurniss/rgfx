...
Assets are bundled by `electron-builder.yml` via `extraResources`. Documentation is no longer bundled — it is hosted online at rgfx.io/docs. The `executableName` is `rgfx-hub` (no spaces) to avoid codesigning issues with Electron helper apps; `productName` remains "RGFX Hub" for display. Electron fuses are configured via `electronFuses` in `electron-builder.yml`. The macOS build uses Apple Developer ID code signing via keychain import in CI. The entitlements file (`entitlements.mac.plist`) grants JIT, unsigned memory, dyld env vars, library validation bypass, and network access required by Electron. The Windows build uses NSIS installer with Azure Trusted Signing via `win.azureSignOptions` in `electron-builder.yml`, using `Invoke-TrustedSigning` PowerShell module. Authentication uses `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET` env vars.
