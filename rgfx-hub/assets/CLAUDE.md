...
Assets are bundled by `forge.config.js` via `extraResource`. Documentation is no longer bundled — it is hosted online at rgfx.io/docs. The macOS build uses Apple Developer ID code signing via `osxSign` in `forge.config.js`. Notarization is handled separately in the CI release workflow using `xcrun notarytool submit --wait --timeout 30m` followed by `xcrun stapler staple`, decoupled from Electron Forge for better timeout control and retry support. The entitlements file (`entitlements.mac.plist`) grants JIT, unsigned memory, dyld env vars, library validation bypass, and network access required by Electron.
