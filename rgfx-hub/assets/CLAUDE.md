...
- `games/` - Game-specific transformer modules (defender.js, galaga.js, galaga88.js, outrun.js, pacman.js, robotron.js, shangon.js, smb.js, ssf2.js, starwars.js, etc.)
  ...
  - robotron.js uses exclusive() wrapper for human-programming flash effect to cancel overlapping async loops. Entity events (spawn/destroy) are handled under proper entity subject with qualifier routing rather than sfx subject. Tracks cumulative score via `lastScore` and displays score delta text on secondary matrices when delta exactly matches 500, 1000, 1500, or 2000. Enforcer and grunt destroy effects use bitmap shatter instead of explode/pulse for visual consistency with the sprite theme. Enforcer spawn sparkle effect removed (redundant with entity bitmap rendering).
...
Assets are bundled by `electron-builder.config.js` via `extraResources` (includes `transformers`, `interceptors`, `firmware`, `mame`, `led-hardware`, and `scripts`). Documentation is no longer bundled — it is hosted online at rgfx.io/docs. The `executableName` is `RGFX Hub` (matching `productName`) so the macOS `.app` bundle and Windows `.exe` display the proper name. Electron fuses are configured via `electronFuses` in `electron-builder.config.js`. The macOS build uses Apple Developer ID code signing via keychain import in CI. The entitlements file (`entitlements.mac.plist`) grants JIT, unsigned memory, and network access required by Electron. The Windows build uses NSIS installer with Azure Trusted Signing via `win.azureSignOptions` in `electron-builder.config.js`, using `Invoke-TrustedSigning` PowerShell module. Authentication uses `AZURE_TENANT_ID`, `AZURE_CLIENT_ID`, `AZURE_CLIENT_SECRET` env vars.
