...
### mame/
MAME core Lua modules (system-level, not user-editable):
...
- `ambilight.lua` - Screen edge color sampling module; samples pixels along screen edges, averages over N frames, emits `rgfx/ambilight/frame` events
- `fft.lua` - Audio spectrum analysis module; performs FFT on MAME audio samples, emits `rgfx/audio/fft` events
...
Assets are bundled by `forge.config.js` via `extraResource`. Documentation is no longer bundled — it is hosted online at rgfx.io/docs. The `executableName` is `rgfx-hub` (no spaces) to avoid codesigning issues with Electron helper apps; `productName` remains "RGFX Hub" for display. Electron fuses are configured via `FusesPlugin`. The macOS build uses Apple Developer ID code signing via `osxSign` in `forge.config.js`. Notarization is handled separately in the CI release workflow using `xcrun notarytool submit --wait --timeout 30m` followed by `xcrun stapler staple`, decoupled from Electron Forge for better timeout control and retry support. The entitlements file (`entitlements.mac.plist`) grants JIT, unsigned memory, dyld env vars, library validation bypass, and network access required by Electron. The Windows build uses Azure Trusted Signing via `forge.config.js`, gated on `AZURE_ENDPOINT` env var (only set in CI). The release workflow installs `Microsoft.ArtifactSigning.Client` via NuGet and configures signtool with the Azure dlib plugin. Two separate signing configs are needed: `packagerConfig.windowsSign` uses `hookFunction` (runs in-process, avoids flag conflicts with `@electron/windows-sign`), while `MakerSquirrel` uses `windowsSign` with serializable options (`signToolPath`, `signWithParams`, `hashes`, `timestampServer`) because Squirrel creates a SEA binary where functions cannot survive serialization. The Squirrel config uses `signToolPath` to point to the system signtool (which supports `/dlib`) instead of electron-winstaller's bundled vendor copy (too old for `/dlib`). The `forge.config.js` file uses `@ts-check` with JSDoc type annotations for type safety. macOS signing uses `@electron/osx-sign` (explicit devDependency).
