...
void TestLedsEffect::render() {
	...
	// Strip layout: 25% segments (Red, Green, Cyan, Purple)
	if (matrix.layoutType == LayoutType::STRIP) {
		...
		canvas.drawRectangle(segmentWidth * 2, 0, segmentWidth, canvasHeight, CRGB(0, 139, 139));
		canvas.drawRectangle(segmentWidth * 3, 0, segmentWidth, canvasHeight, CRGB(255, 0, 255));
	}
	// Matrix layout: 4 quadrants (TL=Red, TR=Green, BL=Cyan, BR=Purple)
	else {
		uint16_t midX = canvasWidth / 2;
		uint16_t midY = canvasHeight / 2;

		canvas.drawRectangle(0, 0, midX, midY, CRGB(255, 0, 0));          // Top-Left: Red
		canvas.drawRectangle(midX, 0, midX, midY, CRGB(0, 190, 0));       // Top-Right: Green
		canvas.drawRectangle(0, midY, midX, midY, CRGB(0, 180, 180));     // Bottom-Left: Cyan
		canvas.drawRectangle(midX, midY, midX, midY, CRGB(160, 0, 160));  // Bottom-Right: Purple
	}
