...
| File | Purpose |
|------|---------|
| `main.cpp` | Entry point with `setup()` and `loop()`. Initializes all subsystems, creates FreeRTOS task for Core 0, runs LED rendering loop on Core 1. Note: per-message UDP logging removed for latency reduction. |
