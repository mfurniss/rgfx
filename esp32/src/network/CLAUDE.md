...
| `udp_video.h/cpp` | Binary video frame receiver. Reassembles fragmented UDP packets into complete RGB24 frames using double-buffered storage. Called from `processUDP()` when packets start with VIDEO_MAGIC byte. API: `peekVideoFrame()` (read without consuming), `consumeVideoFrame()` (clear ready flag), `hasNewVideoFrame()`, `setVideoFrameSize()`. Stats logging uses `snprintf` to avoid heap allocation in the receive hot path. `setVideoFrameSize()` clears stale frame data and resets reassembly state on each start to prevent flash of previous video. |
