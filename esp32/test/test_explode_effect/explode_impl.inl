// ExplodeEffect implementation for testing
// This file is included by test_explode_effect.cpp with mocks already defined
...
// ExplodeEffect class definition
...
void ExplodeEffect::add(JsonDocument& props) {
	...
	if (!props["centerX"].is<float>() && !props["centerX"].is<int>()) {
		// In test environment, log is not available - just return
		return;
	}
	...
	for (uint32_t i = 0; i < particleCount; i++) {
		...
		float powerVariation =
			scaledPower *
			(1.0f + (static_cast<float>(random(-100, 100)) / 100.0f) * (powerSpread / 100.0f));
