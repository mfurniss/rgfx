...
3. **Run `scripts/test-code.sh` as the final step** after implementing any plan. Don't wait for the pre-commit hook to catch issues.
...
7. **Do NOT commit `public-docs/site/`** — it is gitignored. The CI release workflow builds the docs fresh and deploys to Pages.
...
After modifying files in the hub project always run `scripts/test-code.sh`
